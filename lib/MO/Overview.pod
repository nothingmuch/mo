...
familiar with its compile-time/run-time split.

=head1 COMPILE-TIME

The class is the most significant object in the compile-time stack; it
tries to model Perl 6's OO.
...
grammars, as well as ancestors.  It supports both private and public
...
That said, there should be no restriction against meta classes being implemented
